...
public class MessageFactory {
  ...
  public static Error build_error (Display display) //throws IOException {
	{
    if (!display.connected) return build_connection_error (display);
    ...
    if (code >= 128 && code <= 255)
      return build_extension_error (display, data, code, seq_no, bad,
        minor_opcode, major_opcode);

    return new Error (display, Error.ERROR_STRINGS [code], code, seq_no, bad,
      minor_opcode, major_opcode);
  }

  
  public static Event build_event (Display display, int code) 
    //throws IOException 
{
    ...
    // keep code in [0]
    data [0] = (byte) code;
    display.connection.din.Read (data, 1, 31);
  ...
  public static Event build_core_event (Display display, 
    ...
    default:
      throw new Exception ("Unsupported core event: " + code);
    }    
  }
  ...
  public static Event build_extension_event (Display display, 
    ...
    gnu.x11.extension.EventFactory factory = display
      .extension_event_factories [code-64];
    
    if (factory == null) 
      throw new gnu.x11.Error ("Unsupported extension event: " + code);
