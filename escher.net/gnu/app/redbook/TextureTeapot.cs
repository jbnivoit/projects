 ...
 * help output</a>
 */
public class TextureTeapot : gnu.x11.extension.glx.Application {
  private const int IMAGE_SIZE = 32;
  private static Data IMAGE;
  private static byte [] [] IMAGE0;
  private static readonly float [] TEXTURE_SLANTED = {1.0f, 1.0f, 1.0f, 0.0f};
  private static readonly float [] TEXTURE_YZ = {1.0f, 0.0f, 0.0f, 0.0f};
  ...
  private void init_image () {
    ...
    for (int i=0; i<IMAGE_SIZE; i++) {
      IMAGE0 [i] = new byte[4];
      IMAGE0 [i] [0] = (byte)((i<=4) ? 255 : 0);
      IMAGE0 [i] [1] = (byte)((i>=4) ? 255 : 0);
      IMAGE0 [i] [2] = (byte) 0;
      IMAGE0 [i] [3] = (byte) 255;
    }
