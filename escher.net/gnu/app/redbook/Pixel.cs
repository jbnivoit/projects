 ...
 * help output</a>
 */
public class Pixel : gnu.x11.extension.glx.Application {
  private float zoom_factor = 1.0f;


  public Pixel (String [] args) 
    : base (args, BUTTON1_MOTION_BIT | KEYBOARD_BIT | RESIZE_BIT) {
  ...
  protected override void handle_keyboard (int key, int state, int x, int y) {
    ...
    case 'R':
      zoom_factor = 1.0f;
      Console.WriteLine ("zoom-factor reset to 1.0");
      mark_window_dirty ();
      break;
    ...
    case 'z':
      ...
      Console.WriteLine ("zoom-factor: " + zoom_factor);
      break;
  ...
  protected override void handle_motion (int state, int x, int y) {
