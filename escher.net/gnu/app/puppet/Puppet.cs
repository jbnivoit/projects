 ...
 * normal.
 *
 * <p><b>Class-based switch-focus</b> ([(alt tab)] and [(base tab)]):
 ...
 * same WM_CLASS as the window under current focus; and [(base tab)]
 ...
 * help output</a>
 */
public class Puppet : gnu.x11.Application {
  public const int SYSTEM_MODIFIER = gnu.x11.Input.CONTROL_MASK;
  public const int SYSTEM_KEYSYM = Misc.DELETE;
  public const int SWITCH_KEYSYM = Misc.TAB;
  ...
  public const int DELTA_LARGE = 20;
  public const int DELTA_SMALL = 2;
  ...
  public ArrayList focus_so_far = new ArrayList ();
...
//       public void run () { System.err.println ("shutting down..."); }});
  ...
  public void grab_keybut () {
    ...
    root.grab_key_ignore_locks (SWITCH_KEYSYM, gnu.x11.Input.META_MASK, true,
      Window.ASYNCHRONOUS, Window.ASYNCHRONOUS);
    root.grab_key_ignore_locks (SWITCH_KEYSYM,
      gnu.x11.Input.META_MASK|gnu.x11.Input.SHIFT_MASK, true, Window.ASYNCHRONOUS,
      Window.ASYNCHRONOUS);

    // rotate to next normal window - different class 
    root.grab_key_ignore_locks (SWITCH_KEYSYM, gnu.x11.Input.SUPER_MASK, true, 
      Window.ASYNCHRONOUS, Window.ASYNCHRONOUS);
    root.grab_key_ignore_locks (SWITCH_KEYSYM,
      gnu.x11.Input.SUPER_MASK|gnu.x11.Input.SHIFT_MASK, true, Window.ASYNCHRONOUS,
      Window.ASYNCHRONOUS);

    // rotate to next normal window - same class
    root.grab_key_ignore_locks (SWITCH_KEYSYM, gnu.x11.Input.ALT_MASK, 
      true, Window.ASYNCHRONOUS, Window.ASYNCHRONOUS);
    root.grab_key_ignore_locks (SWITCH_KEYSYM,
      gnu.x11.Input.ALT_MASK|gnu.x11.Input.SHIFT_MASK, true, Window.ASYNCHRONOUS,
      Window.ASYNCHRONOUS);

    // CLICK-TO-FOCUS
    root.grab_button_ignore_locks (Window.ANY_BUTTON,
      gnu.x11.Input.CONTROL_MASK, true, Event.BUTTON_PRESS_MASK,
      Window.ASYNCHRONOUS, Window.ASYNCHRONOUS, Window.NONE,
      Cursor.NONE);

    // close / delete
    root.grab_button_ignore_locks (Window.ANY_BUTTON,
      gnu.x11.Input.CONTROL_MASK | gnu.x11.Input.META_MASK, true,
  ...
  public bool key_do_change_geometry () {
  ...
  public bool key_do_critical_operation () {
    switch (keysym) {
  ...
  public bool key_do_jump_lanuch_or_argument () {
  ...
  public void key_dump_info () {
    Console.WriteLine ("input focus: " + focus);
    Console.WriteLine ("mouse at: " + root.pointer ().root_position ());

    if (!argument_present) return; // `dump-basic-info'

    if (argument_negative) {    // `dump-hidden-windows'"
      Console.WriteLine ("all hidden clients: ");

      for (int i=0;i<clients.Count;i++) {
        Client c = (Client) clients[i];
        
        if (c.state == InternalState.HIDDEN)
          Console.WriteLine (c);
      }

    } else                      // `dump-all-windows'
      Console.WriteLine ("all clients: " + clients);
  }
  ...
  public bool key_process_no_prefix () {
    switch (keysym) {
    ...
    case Misc.KP_BEGIN:
      key_click_button (gnu.x11.Input.BUTTON1);
      return true;

    case Misc.KP_DIVIDE:
      key_click_button (gnu.x11.Input.BUTTON2);
      return true;

    case Misc.KP_MULTIPLY:
      key_click_button (gnu.x11.Input.BUTTON3);
      return true;
  ...
  public bool key_process_prefix0 () {
    switch (prefix0) {
    case Misc.BACKSPACE:
    ...
    default:
      throw new gnu.x11.Error ("Unhandled prefix0: " + prefix0);
    }
  }


  public bool key_process_prefix1 () {
    throw new gnu.x11.Error ("Unhandled prefix1: " + prefix1);
  }
  ...
  public bool system_key_pressed, focus_key_pressed;
  ...
  public void read_and_dispatch_event () {
    ...
    display.check_error ();
    ArrayList other_events = display.connection.pull_all_events ();
    
    for (int i=0;i<other_events.Count;i++) {
      Event evt = (Event) other_events[i];

      if (evt.code () == DestroyNotify.CODE) {	
        Client client = (Client) Client.intern (display, 
          evt.window_id ());
        client.early_destroyed = true;

      } else if (evt.code () == UnmapNotify.CODE) {
        Client client = (Client) Client.intern (display, 
          evt.window_id ());
  ...
  public void scan_children () {
	...
	client.state = InternalState.NORMAL;
	// in case someone screws this up
	client.set_wm_state (Window.WMState.NORMAL);
      }


      if (client.state == InternalState.NORMAL || client.state == InternalState.HIDDEN)
	manage (client);
    }
  }
  ...
  public void when_configure_request (ConfigureRequest evt) {
     ...
     * have this problems?)
     */
    client.configure (evt.changes ());
    
    client.set_geometry_cache (evt.rectangle ());


    // we choose to give it focus if it is normal and it raises
    if (client.state == InternalState.NORMAL
      && evt.stack_mode () == Window.Changes.ABOVE)

      set_focus (client, false);
  }


  public void when_destroy_notify (DestroyNotify evt) {    
    Client client = (Client) Client.intern (display, evt.window_id ());
    give_up_focus (client);
  ...
  public bool alt_down, control_down, meta_down, shift_down, base_down;
  public int prefix0, prefix1;  
  public int argument;
  public bool argument_present;
  public bool argument_negative;
  ...
  public void when_key_release (KeyRelease evt) {
    keycode = evt.detail ();
    int keystate = evt.state ();
    keysym = display.input.keycode_to_keysym (keycode, keystate);
  ...
  public void when_property_notify (PropertyNotify evt) {
    Atom atom = evt.atom (display);
    ...
    if (atom == wm_colormap_windows
      || atom == wm_protocols)
      throw new gnu.x11.Error ("unhandled property notfiy: " + atom);
  ...
  public void when_map_request (MapRequest evt) {
  ...
  public void when_map_notify (MapNotify evt) {
    Client client = (Client) Client.intern (display, evt.window_id ());
    if (client.early_unmapped || client.early_destroyed) return;
     ...
     */
    if (client.state != InternalState.NO_FOCUS) set_focus (client, false);
  }
  ...
  public void when_unmap_notify (UnmapNotify evt) {
    ...
    // they withdraw it
    if (client.state != InternalState.HIDDEN) {
       ...
       */
      client.state = InternalState.UNMANAGED;
